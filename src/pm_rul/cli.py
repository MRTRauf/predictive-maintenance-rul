...
def _health_status() -> str:
    ...
    if missing:
        return "missing: " + ", ".join(str(p) for p in missing)
    return "ok"
...
@app.command("ingest-cmapss")
def ingest_cmapss_command(subset: str = "FD001", force: bool = False) -> None:
...
def train_baseline_command(
    ...
    _print_training_summary(results["metrics"], paths)


def _print_training_summary(metrics: dict, paths: dict) -> None:
    console = Console()
